...
if prompt_update_file "$REACT_ENV_FILE"; then
  CURRENT_VITE_EX_CP_PUBLIC_KEY="$(read_env_value "$REACT_ENV_FILE" "VITE_EX_CP_PUBLIC_KEY")"
  if [[ -z "$CURRENT_VITE_EX_CP_PUBLIC_KEY" ]]; then
    CURRENT_VITE_EX_CP_PUBLIC_KEY="$(read_env_value "$REACT_ENV_FILE" "NEXT_PUBLIC_EX_CP_PUBLIC_KEY")"
  fi

  VITE_EX_CP_PUBLIC_KEY="$(prompt_required "VITE_EX_CP_PUBLIC_KEY" "Public key" false "$CURRENT_VITE_EX_CP_PUBLIC_KEY")"

  cat > "$REACT_ENV_FILE" <<EOF
VITE_EX_CP_PUBLIC_KEY=$(escape_env_value "$VITE_EX_CP_PUBLIC_KEY")
EOF
